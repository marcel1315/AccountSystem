...
public class AccountService {
    ...
    public AccountDto createAccount(Long userId, Long initialBalance) {
        final int randomAccountNumberRetryCount = 100;
        AccountUser accountUser = getAccountUser(userId);

        validateCreateAccount(accountUser);

        String newAccountNumber = getNewAccountNumber(randomAccountNumberRetryCount);
    ...
    private String getNewAccountNumber(int retry) {
        int i = 0;

        String newAccountNumber;
        do {
            long randomNumber = new Random().nextLong(1_000_000_000L, 9_999_999_999L);
            newAccountNumber = String.valueOf(randomNumber);
            i++;
            if (i > retry) {
                throw new AccountException(ACCOUNT_CREATION_FAIL);
            }
